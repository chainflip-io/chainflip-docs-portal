...
function SocialCard({ Icon, title, description, href }: CardProps) {
  return (
    <a
      href={href}
      rel="noopener noreferrer"
      className="flex items-center p-4 rounded-2xl shadow-md bg-neutral-100 border-1 dark:bg-neutral-800 border-neutral-200 dark:border-neutral-700 cursor-pointer hover:shadow-lg hover:shadow-gray-300 dark:hover:shadow-gray-800 transition-shadow duration-300 ease-in-out"
    >
      {/* Icon */}
      ...
      <div className="h-16 ms-4 flex flex-col justify-center">
        ...
        <h6 className="text-sm sm:text-xs overflow-hidden text-ellipsis line-clamp-2">
          {description}
        </h6>
      </div>
    </a>
...
export default function SocialCards({ className }: { className?: string }) {
  return (
    <div className={classnames('p-6 space-y-6', className)}>
      {/* Row of three cards */}
