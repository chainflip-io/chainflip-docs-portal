...
1. The user, through a [Broker](/brokers#brokers) (discussed below), has an extrinsic submitted to the State Chain which includes all required swap data, such as their Destination Chain and Destination Address.
...
See the [Vault design](/protocol/vaults/bitcoin-vault-design) documentation for more information on how Deposit Channels are handled during Vault rotations on each chain type.
...
<Callout type="info">
  The cost Brokers pay to open _Deposit Channels_ is **very** cheap. See the \$FLIP balance from
  [Chainflip's
  Broker](https://scan.chainflip.io/brokers/cFLRQDfEdmnv6d2XfHJNRBQHi4fruPMReLSfvB8WWD2ENbqj7) —
  used to power our Swapping app.
</Callout>
