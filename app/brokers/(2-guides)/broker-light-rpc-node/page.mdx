...
## Running the Broker light-rpc node using `chainflip-node` binary
...
The main goal of Broker light-rpc mode is to enable brokers to track only latest chain blocks data and in particular be able to make rpc calls that require signed extrinsics such as
[`broker_request_swap_deposit_address`](/brokers/broker-api/requests#broker_request_swap_deposit_address), [`broker_withdraw_fees`](/brokers/broker-api/requests#broker_withdraw_fees) etc ...

* The first step is to inject the **Broker Signing Key** inside the chainflip node. This can be achieved using this command
```bash
/usr/local/bin/chainflip-node key insert \
        ...
        --scheme=sr25519
```
NB: please change `/etc/chainflip-keys/broker/signing_key_file` with location of your broker signing key file

* Then run the node in **light-rpc mode**, by simply calling the `chainflip-node` binary with some special command line arguments that makes the node prune blocks minimising disk space.
This will require about 20GB of disk space.
