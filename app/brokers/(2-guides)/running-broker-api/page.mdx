...
Brokers need to run the client for the API themselves, as the Broker holds keys used to sign extrinsics and collect any fees set for [Deposit Channels](/brokers/broker-deposit-channels-swaps#deposit-channels). The API client works by pointing it to an RPC node — also run by the same Broker, ideally.
...
<Callout type="warning">
  Make sure the signing key file is not exposed. Access to this key gives full control over the
  Broker account.
</Callout>
...
<Callout type="warning">
  Avoid exposing the broker-api publicly with these settings, as anyone could redeem your earned
  fees!
</Callout>
