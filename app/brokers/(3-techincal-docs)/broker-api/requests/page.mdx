...
- `refund_parameters`: (Optional) [Slippage protection](/protocol/swapping-basics#slippage-protection) for swaps through the channel as JSON object: `{"retry_duration": <number>, "refund_address": <address>, "min_price": <hex_string>, "max_oracle_price_slippage": <number | undefined>}`. Where `retry_duration` is a number of blocks, `refund_address` is an [address](#addresses), and `min_price` is a [price](#price), and `max_oracle_price_slippage` is basis points.
...
Where every affiliate broker is defined as follows:
```json
{
	"account": <AccountId>,
	"bps": <BasisPoints>,
...
- `extra_parameters`: JSON object with different data depending on the chain of the input asset.
  - `chain`: The chain of the input asset. Use only one of the following, then provide the corresponding parameters:
    - `Bitcoin`:
      ...
      - `max_oracle_price_slippage`: [Live price protection](/protocol/swapping-basics#maximum-oracle-price-slippage) in basis points (100th of a percent) as a `u8` number (set to 255 to disable).
    - `Ethereum` or `Arbitrum`:
      - `input_amount`: Amount of the input asset in the smallest unit.
      - `refund_parameters`: JSON object: `{"retry_duration": <number>, "refund_address": <address>, "min_price": <hex_string>, "max_oracle_price_slippage": <number | undefined>}`. See [broker_request_swap_deposit_address](#broker_request_swap_deposit_address) for more details.
    - `Solana`:
      - `from`: [Address](#addresses) of the sender.
      - `seed`: Seed used by the program to derive the [event data account](/brokers/vault-swaps-api/solana#swap-event-data-account). The maximum valid length is 32-bytes.
      - `input_amount`: Amount of the input asset in the smallest unit.
      - `refund_parameters`: JSON object: `{"retry_duration": <number>, "refund_address": <address>, "min_price": <hex_string>, "max_oracle_price_slippage": <number | undefined>}`. See [broker_request_swap_deposit_address](#broker_request_swap_deposit_address) for more details.
...
Bind the broker account to a given withdrawal address. The broker can only withdraw the fees to this address from this point on. This bound address cannot be changed once set.

Parameters:
- `address`: The ethereum address which the broker account wants to bind to as fee withdrawal address.

Return: The transaction hash of state chain extrinsic.
...
curl 'http://localhost/' \
  -H 'accept: application/json' \
  -H 'content-type: application/json' \
  --data-raw '{"id": 1, "jsonrpc": "2.0", "method": "broker_bind_fee_withdrawal_address", "params": {"address": "0x3499b9b27ec38fe019299498958c451d294f6b5e"}}' | jq
```
...
{
  "jsonrpc": "2.0",
  "result": "0x2b130acc1b3d56f8caff1e1adc32b8fbcc25c5ce0d9daa559d073e438c136ce1",
  "id": 1
}
```
