...
The Slippage Protection `fillOrKillParams` object sets a [slippage protection](/protocol/swapping-basics#slippage-protection) when initiating a swap.
This protects the swapper against price changes between a quote and the execution of a swap.
If the minimum price or oracle slippage cannot be satisfied with the available liquidity during the specified number of blocks,
