...
<Callout type="warning">
  If overriding live price slippage protection - the tolerance should account for network fees and
  broker fees. For example, if the network fee is 0.1% and the broker fee is 0.15% - the live price
  slippage tolerance should be set to at least 0.25% to avoid refunds. The recommended value
  returned from the quote
  <strong>already</strong> accounts for these fees.
</Callout>

<Callout type="warning">
  Slippage tolerance is applied relative to the quoted price unlike live price slippage which is
  applied relative to the global index price. Prices are checked on the AMM level and do not include
  deposit or broadcast fees.
</Callout>
