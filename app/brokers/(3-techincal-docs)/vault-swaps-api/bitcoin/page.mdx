...
<Callout type="info">
  Bitcoin Vault Swaps have a unique requirement compared to other chains:
  Brokers must first open a [Private Bitcoin
  Channel](/brokers/broker-api/requests#broker_open_private_btc_channel).
  Private channels prevent spam, and allow the Broker to uniquely assign
  commission to itself and its Affiliates. Opening a Private Channel is a
  **one-time** operation.
</Callout>

Creating a Bitcoin Vault Swap follows the same pattern mentioned in the [Vault Swaps introduction](/brokers/broker-vault-swaps):
...
These must be included in a Bitcoin transaction containing, as its first three outputs, _exactly_ the following UTXOs, in this order:
...
Per the Bitcoin protocol, the change UTXO amount _cannot_ be zero and must be larger than Bitcoin's dust threshold for the change address type. Therefore it is not possible to swap the entire amount of the input UTXOs, a dust amount needs to be sent to the change address.
...
const txId = (await btcClient.sendRawTransaction(signedTx.hex)) as
  | string
  | undefined;
...
<Callout type="warning">
  Note that, unlike other chains, Bitcoin Vault Swaps *expire* after two Epoch
  rotations. At the time of writing, this takes between 3 and 6 days.
  Nonetheless, it is recommended that Vault Swap payloads are assumed to be
  ephemeral unless you know what you are doing and are willing to take the risk
  of losing funds if an expired transaction is submitted.
</Callout>
...
<Callout type="warning">
  You should not need to build this encoding yourself. Always prefer to use the
  [`broker_request_swap_parameter_encoding()` RPC](/brokers/broker-vault-swaps)
  to encode the swap parameters. It's more convenient and more reliable.
</Callout>
...
| --------------------------- | -------------------------------------------------------------------------------------------------------------------- | --------------- |
| `version`                   | Encoding version number. Currently `1`                                                                               | `u8`            |
| `output_asset`              | Destination [asset ID](/brokers/vault-swaps-api/encoding-reference#supported-chain-and-asset-notation).              | `u8`            |
| `output_address`            | Encoded [address](/brokers/broker-api/types#addresses)                                                               | `Vec<u8>`       |
...
| `max_oracle_price_slippage` | Maximum oracle price slippage in basis points (100th of a percent). Set to 255 to disable this check.                | `u8`            |
...
| `affiliates`                | List of up to 2 [affiliate short IDs](/brokers/broker-api/requests#broker_register_affiliate) and fee (basis points) | `Vec<(u8, u8)>` |

<Callout type="info">
  The encoded data in `nulldata_payload` returned by
  [broker_request_swap_parameter_encoding](/brokers/broker-api/requests#broker_request_swap_parameter_encoding)
  does not include any OP codes. Depending on how you build your transaction,
  you may need to add these manually. First opcode must be `OP_RETURN` followed
  by either `OP_PUSHBYTES_X` (1..=75) or `OP_PUSHDATA1` (76).
</Callout>


## SDK Usage
if you are using the typescript chainflip SDK, you can find a working example in the [SDK repository](https://github.com/chainflip-io/chainflip-sdk-monorepo/blob/main/packages/examples/src/bitcoinVaultSwap.ts).
