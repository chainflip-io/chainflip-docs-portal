...
Initiating a [Vault Swap](/brokers/broker-vault-swaps) on an EVM chain (currently Ethereum or Arbitrum) requires making a smart contract call to Chainflip's Vault contract. The contract addresses can be found in [Testnet addresses](/protocol/supported-chains-assets/testnet-addresses) and [Mainnet addresses](/protocol/supported-chains-assets/mainnet-addresses).

EVM Vault Swaps follow the same pattern mentioned in the [Vault Swaps introduction](/brokers/broker-vault-swaps):
...
<Callout type="info">
  Note that if `source_token_address` is present, the returned address is the
  contract address of the ERC-20 token. This requires approval / whitelisting by
  the sender, *before* submitting the vault swap.
</Callout>
...
| -------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------- |
| `dstChain`     | Destination [chain ID](/brokers/vault-swaps-api/encoding-reference#supported-chain-and-asset-notation) for the swap.                                                                                                                                                                                                                                                                                          | `uint32`  |
...
| `dstToken`     | Destination [token](/brokers/vault-swaps-api/encoding-reference#supported-chain-and-asset-notation)                                                                                                                                                                                                                                                                                                           | `uint32`  |
...
| `cfParameters` | Additional metadata for additional features. It cannot be empty and must be correctly encoded according to [Chainflip Parameters](/brokers/vault-swaps-api/encoding-reference#chainflip-parameters-cf_parameters).                                                                                                                                                                                            | `bytes`   |

## SDK Usage
if you are using the typescript chainflip SDK, you can find a working example in the [SDK repository](https://github.com/chainflip-io/chainflip-sdk-monorepo/blob/main/packages/examples/src/ethereumVaultSwap.ts).
