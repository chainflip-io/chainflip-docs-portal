...
<Callout type="info">
  Note that if `source_token_address` is present, the returned address is the contract address of
  the ERC-20 token. This requires approval / whitelisting by the sender, *before* submitting the
  vault swap.
</Callout>
