...
Initiating a Solana [Vault Swap](/brokers/broker-vault-swaps) requires making a program call to Chainflip's Swap Endpoint program. The program addresses can be found in [Testnet addresses](/protocol/supported-chains-assets/testnet-addresses) and [Mainnet addresses](/protocol/supported-chains-assets/mainnet-addresses).

Solana Vault Swaps follow the same pattern mentioned in the [Vault Swaps introduction](/brokers/broker-vault-swaps):
...
| `dstChain`      | Destination [chain ID](/brokers/vault-swaps-api/encoding-reference#supported-chain-and-asset-notation)                                                                                                                                                                                                                                                                                                        | `u32`                          |
...
| `dstToken`      | Destination [token](/brokers/vault-swaps-api/encoding-reference#supported-chain-and-asset-notation).                                                                                                                                                                                                                                                                                                          | `u32`                          |
| `ccmParameters` | Additional metadata for swaps with Cross-Chain Messaging. Should be set to `None` for simple swaps. See [CCM support](/protocol/cross-chain-messaging/sol-ccm) for more details.                                                                                                                                                                                                                              | `Option<(message, gasAmount)>` |
...
| `cfParameters`  | Additional metadata for additional features. It cannot be empty and must be correctly encoded according to [Chainflip Parameters](/brokers/vault-swaps-api/encoding-reference#chainflip-parameters-cf_parameters).                                                                                                                                                                                            | `Vec<u8>`                      |
...
| ------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------ |
| `dataAccount`             | Chainflip's Vault Data Account. See [Mainnet addresses](/protocol/supported-chains-assets/mainnet-addresses).                                                                                                                          | Data Account |
...
| `swapEndpointDataAccount` | Chainflip's Swap Endpoint Data Account. See [Mainnet addresses](/protocol/supported-chains-assets/mainnet-addresses).                                                                                                                  | Data Account |
...
| `tokenSupportedAccount`   | Chainflip's Token Supported Account for the specific token. This is only needed when swapping tokens. Mainnet addresses can be found [here]( [Mainnet addresses](/protocol/supported-chains-assets/mainnet-addresses)).                | Data Account |
| `mint`                    | Address of the SPL-token to swap. The token should be supported by Chainflip, otherwise the transaction will revert. Mainnet addresses can be found [here]( [Mainnet addresses](/protocol/supported-chains-assets/mainnet-addresses)). | Data Account |

## SDK Usage
if you are using the typescript chainflip SDK, you can find a working example in the [SDK repository](https://github.com/chainflip-io/chainflip-sdk-monorepo/blob/main/packages/examples/src/solanaVaultSwap.ts).
