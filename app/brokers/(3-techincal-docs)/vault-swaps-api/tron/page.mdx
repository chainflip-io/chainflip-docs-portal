...
The response fields differ from EVM:
...
| `to`                   | Hex-encoded destination address. For TRX: the Chainflip Vault. For TRC20: the token contract address. Must be converted to Tron base58 format before use. |
...
| `source_token_address` | (Optional) Hex-encoded TRC20 token contract address. Present only for TRC20 swaps.                                              |
