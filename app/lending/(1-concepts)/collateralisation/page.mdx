...
- When soft liquidation is triggered automatically, the system only swaps small chunks until LTV returns to healthy levels.
- When a borrower triggers manual liquidation, the entire collateral balance is swapped. Any excess remains in the collateral balance.

This distinction is important because manual liquidation will unwind the *entire* position immediately, while soft liquidation only corrects the LTV.
</Callout>
