...
Under the unified pool model, supplied collateral is deployed into the lending pool and earns interest from loans taken out against it. BTC collateral additionally earns Boost fees. See [Supply as Collateral](/lending/utilisation#supply-as-collateral-the-unified-pool-model) for details.

To avoid liquidation, borrowers should monitor their LTV and add collateral or repay debt as needed. The lending app provides warnings when LTV approaches unsafe levels.
...
However, if at any point during manual liquidation LTV crossess soft/hard liquidation threshold, it will lose its "manual" status, and liquidation fee will still apply (though only on the portion of collateral swapped during the non-manual period).
