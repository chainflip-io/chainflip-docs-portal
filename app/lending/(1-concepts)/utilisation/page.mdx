...
Default value for the loan coverage is 100%, which is quite conservative, as there are almost no scenarios in which 100% of loans need to be liquidated at the same time. Partial liquidations can restore loans to healthy LTV levels without requiring full liquidations.

Loan coverage value can be updated through Chainflip governance, similar to other lending parameters. 
