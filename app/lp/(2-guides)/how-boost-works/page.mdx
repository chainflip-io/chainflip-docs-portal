...
When the network sees a prewitnessed deposit on a _boostable_ channel (one that specifies a non-zero boost
fee limit), it checks if there are enough _available_ funds in the given asset's pools that satisfy the fee limit.
