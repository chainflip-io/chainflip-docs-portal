---
title: LP API Requests

description: All the endpoints used in LP API
---
...
<Callout type="error">
  This RPC is deprecated as of 1.9. Please use the
  [`lp_request_liquidity_deposit_address`](#lp_request_liquidity_deposit_address) RPC instead.
</Callout>
...
- [`asset`](/lp/lp-api/types#asset): The asset you wish to deposit.
- [`wait_for`](/lp/lp-api/types#wait-for) (Optional)
- [`boost_fee`](/lp/lp-api/types#boost-fee) (Optional): The maximum amount you are willing to pay to [boost](/lp/how-boost-works) the deposit, expressed as basis points.

[Response](/lp/lp-api/types#wait-for-result):

- The encoded deposit [address](/lp/lp-api/types#addresses) that you should send the deposit to.
...
- An object containing The encoded deposit [address](/lp/lp-api/types#addresses) and the block number of the deposit chain at which the deposit address will expire.
...
- [`chain`](/lp/lp-api/types#chain): The external chain the refund address exists on.
- [`address`](/lp/lp-api/types#addresses): The refund address for the specified external chain. Any assets associated with that chain will be transferred there if a refund is needed, e.g. "USDC" would be transferred to the address associated with "Ethereum".

Return:

- The transaction hash of the extrinsic that registered the new withdrawal [address](/lp/lp-api/types#foreign-chain-addresses).
...
- [`asset_amount`](/lp/lp-api/types#amount): The amount to be withdrawn.
- [`asset`](/lp/lp-api/types#asset): The withdrawn asset.
- [`destination_address`](/lp/lp-api/types#addresses): The address to send the withdrawn funds to.
- [`wait_for`](/lp/lp-api/types#wait-for)
...
- [`asset_amount`](/lp/lp-api/types#amount): The amount to be transferred.
- [`asset`](/lp/lp-api/types#asset): The asset to be transferred.
- [`destination_account`](/lp/lp-api/types#account): The account to transfer the funds to.
...
- [`base_asset`](/lp/lp-api/types#asset)
- [`quote_asset`](/lp/lp-api/types#asset)
- [`id`](/lp/lp-api/types#order-ids)
- `tick_range` (Optional): A JSON array of two [Ticks](/lp/lp-api/types#tick), representing the lower and upper bound of the order's price range. Must be specified if no range order with the specified `id` exists in this pool. If not specified, the tick range of the existing order with the same `id` will be used.
- [`size_change`](/lp/lp-api/types#order-updates)
- [`wait_for`](/lp/lp-api/types#wait-for)
...
- [`id`](/lp/lp-api/types#order-ids)
- `tick_range` (Optional): A JSON array of two [Ticks](/lp/lp-api/types#tick), representing the lower and upper bound of the order's price range. Must be specified if no range order with the specified `id` exists in this pool. If not specified, the tick range of the existing order with the same `id` will be used.
- [`size`](/lp/lp-api/types#range-order-size)
- [`wait_for`](/lp/lp-api/types#wait-for)
...
- [`base_asset`](/lp/lp-api/types#asset)
- [`quote_asset`](/lp/lp-api/types#asset)
- [`side`](/lp/lp-api/types#side)
- [`id`](/lp/lp-api/types#order-ids)
- [`tick`](/lp/lp-api/types#tick) (Optional): The price of the limit order.
- [`amount_change`](/lp/lp-api/types#amount): The amount of assets to add or remove from the limit order. For "buy" orders, this is measured in the quote asset, and for "sell" orders, this is measured in the base asset.
...
- [`id`](/lp/lp-api/types#order-ids)
- [`tick`](/lp/lp-api/types#tick) (Optional): The price of the limit order.
- [`sell_amount`](/lp/lp-api/types#amount): The amount of assets the limit order should sell. For "buy" orders, this is measured in the quote asset, and for "sell" orders, this is measured in the base asset.
- `dispatch_at` (Optional): If specified this RPC instead of submitting an extrinsic that does the specified operation, will submit an extrinsic that schedules the operation to occur at the specified block number. Note the RPC doesn't wait until the operation is scheduled, but only for the extrinsic (to the extent specified by the `wait_for` argument). If the `dispatch_at` block has already elapsed, this method returns an error.
- [`wait_for`](/lp/lp-api/types#wait-for)
...
- A chronological list of all the [updates](/lp/lp-api/types#order-updates) that were made as part of the operation. Each item in the list is a JSON object:
...
This RPC returns the free balances of your LP account as of the latest finalized block. Note that 'Free Balances' does _not_ include any assets currently active in LP orders or in boost pools.
...
Please see [Redeeming Funds & Retiring](/validators/mainnet/funding#redeeming-funds--retiring) for more details on redeeming.
Returns the transaction hash of the extrinsic that requested the redemption.

Parameters:

- [`redeem_address`](/lp/lp-api/types#addresses): The Ethereum address to send the redeemed funds to.
- [`exact_amount`](/lp/lp-api/types#asset) (Optional): The amount of flip to redeem. If not specified, all $FLIP will be redeemed.
- [`executor_address`](/lp/lp-api/types#addresses) (Optional): Restricts the `executeRedemption` function call in the [Gateway contract](/protocol/funding-state-chain-account#state-chain-flip-gateway) to only be callable by the specified address for this redemption. If not specified, anyone can execute the redemption.
...
Redeem an exact [`amount`](/lp/lp-api/types#amount):
...
This RPC allows you to initiate an [internal swap](/lp/internal-swaps).

Parameters:

- [`amount`](/lp/lp-api/types#amount): The amount of input asset to swap.
- [`input_asset`](/lp/lp-api/types#asset): The asset to swap from.
- [`output_asset`](/lp/lp-api/types#asset): The asset to swap into.
- `retry_duration`: A number of blocks for which the swap will be retried if it fails (see [FoK](/protocol/swapping-basics#slippage-protection) section).
- `price_limits`: minimum [price](#hex-price) and maximum oracle slippage (basis points) in a Json object `{"min_price": <hex_string>, "max_oracle_price_slippage": <number | undefined>}` (see [slippage protection](/protocol/swapping-basics#slippage-protection) section).
- `dca_params`: (optional) `DcaParameters`, which determines whether (and how) the swap should be split into chunks (see [DCA](/protocol/swapping-basics#dollar-cost-average-dca-improving-price) section).
- [`wait_for`](/lp/lp-api/types#wait-for)
