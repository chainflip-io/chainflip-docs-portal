...
- `retry_duration`: A number of blocks for which the swap will be retried if it fails (see [FoK](/brokers/swapping-basics#slippage-protection-minimum-accepted-price) section).
- [`min_price`](#hex-price): The minimum price to swap at (see [FoK](/brokers/swapping-basics#slippage-protection-minimum-accepted-price) section).
- `dca_params`: (optional) `DcaParameters`, which determines whether (and how) the swap should be split into chunks (see [DCA](/brokers/swapping-basics#dollar-cost-average-dca-improving-price) section).
- [`wait_for`](/lp/lp-api/types#wait-for)
