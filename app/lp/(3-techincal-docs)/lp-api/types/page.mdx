...
If, at the time the deposit is witnessed by the network, boost liquidity at or below the desired boost fee is sufficient to cover the _entire deposit_, the deposit will be boosted. This means the deposit will be credited after a single confirmation, rather than waiting for the usual [safety margin](/protocol/ingress-witnessing-deposits#the-witness-process) or finality window.
...
Some subscriptions take an optional parameter `notification_behaviour` , this allows you to control under what condition the subscription will return events. It can have these values:

- `Finalized`: This will cause the subscription to send notifications about finalized blocks
- `Best`: This will cause the subscription to send notifications about best blocks. In the case of a re-org it might drop events.
- `New`: This will cause the subscription to send notifications about new blocks. In the case of a re-org it might duplicate events. It is responsability of the user to de-duplicate the events.
