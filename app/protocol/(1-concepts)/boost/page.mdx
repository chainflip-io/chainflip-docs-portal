...
2. The protocol [opens a *boosteable* Swap Deposit Channel](/brokers/broker-deposit-channels-swaps#opening-deposit-channels) and generates a Bitcoin deposit address
3. The user sends the desired amount of BTC to the generated deposit address
4. The protocol attempts to [*Boost* the incoming deposit](/lp/how-boost-works#boosting-a-deposit), by checking the available *Boost* liquidity provided by LPs
...
- The protocol will only attempt to boost a *second* deposit if the first deposit has been [finalised](/protocol/ingress-witnessing-deposits#the-witness-process).