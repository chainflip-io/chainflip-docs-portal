...
3. Sending ERC-20 \$FLIP funds from the _StateChainGateway_ contract when a Validator or other State Chain account holder **redeems $FLIP** from their State Chain account
4. Rotating aggregate keys or sweeping funds in the Vaults

As with _Ingress_, the process for _Egress_ differs slightly from case to case but shares a common process.
