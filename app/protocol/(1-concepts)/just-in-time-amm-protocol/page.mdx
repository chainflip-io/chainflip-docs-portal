...
<Callout type="info">
  The Just-In-Time (JIT) AMM was introduced to the protocol design on the Feb 2022. Check our blog
  post here: [Just in Time AMM](https://blog.chainflip.io/just-in-time-jit)
</Callout>
...
- **Range orders** work as they normally do in Uniswap v3. Liquidity remains deployed until the LP submits an extrinsic which removes it from the pool and returns the remaining funds and fees to their [liquidity account](/lp/lp-account) balance.
...
- The User (BTC buyer) opens a [Deposit Channel](/brokers/broker-deposit-channels-swaps#deposit-channels) connected with their destination address and swap intents for BTC. The user [initiates the swap](/protocol/how-swapping-works#native-swap-flow) by depositing 10,000 USDC.
...
- Chainflip requires 5 Ethereum block confirmations to consider a deposit transaction ‘final.’ It is therefore estimated that the swap will be executed in 10 Chainflip State Chain blocks (around 60 seconds). Market Makers now must include any State Chain limit order or range order updates in those 10 blocks to compete for the trade.
...
- Market Maker A might now make some [withdrawals or deposits to their LP position](/lp/lp-account) to rebalance their portfolio to prepare to capture future opportunities, while Market Maker B is ready to fill another order in the immediate future. The next one is just 1 Ethereum block away.
