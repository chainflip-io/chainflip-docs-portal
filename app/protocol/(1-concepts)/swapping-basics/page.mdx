...
Chainflip swaps are _fire-and-forget_, meaning once a deposit is made, users don't need to do anything else for the swap to take place. In a nutshell, the swapping process starts when a deposit is made to the _State Chain_ through a process called [Ingress](/protocol/ingress-witnessing-deposits).

The [State Chain](/protocol/protocol-overview#the-state-chain) is an application-specific blockchain designed for the Chainflip protocol, making cross-chain swap transactions efficient.
Whenever funds are deposited, the State Chain witnesses the deposit, where they are bundled and processed to be swapped in the [Just-In-Time AMM](/protocol/just-in-time-amm-protocol) protocol — all in the same block.
Once the funds are swapped, the new assets are ready to be sent to the destination address by a process called [Egress](/protocol/egress-broadcasting-funds).
To learn more on how Chainflip swaps work on the protocol level, check the [Native Swap Flow](/protocol/how-swapping-works) guide.

At a high level, the process can be described with the following steps:

<Steps>
    ### Initiating a swap
    This first step can be done through the [Swapping app](https://swap.chainflip.io/) (frontend), the [Javascript SDK](/brokers/javascript-sdk), or directly using the [Chainflip CLI client](/brokers/broker-api/requests#broker_request_swap_deposit_address). There are two primary methods to start a Chainflip Swap:
    * **Generating a Deposit Address**: This method involves creating a unique deposit address where the user can send the funds. Once the deposit is made, the swap process is automatically triggered. See [Request Deposit Address](/brokers/javascript-sdk/functions#requestdepositaddressv2) method.
    * **Direct call to the Chainflip Vault:** This method is a function call to a smart contract for EVM chains and a program call to a program for Solana, including a payload. This makes *aggregation* easier for application developers (think DEX Aggregators). For more details see [Vault swaps](/brokers/broker-vault-swaps).

    ### Witnessing the deposit
    The Validator network will monitor open [Deposit Channels](/brokers/broker-deposit-channels-swaps#deposit-channels) for incoming transactions and Vault Contracts for swap calls. Upon seeing a deposit for a Swap, it will be witnessed and registered on the State Chain.
    ...
    > [!TIP]
    >
    > At this point, you can use the [Block Explorer](https://scan.chainflip.io/swaps) to see the swap '_in progress_'.

    ### Processing the swap
    Once the swap is Witnessed it is ready to be swapped in the [Just-In-Time AMM](/protocol/just-in-time-amm-protocol) protocol. Swaps are automatically bundled and processed so that LPs can bid on swaps in the same block where they occur.\
...
1. A user initiates a swap by depositing **10,000 USDC** **to buy** **BTC**. The deposit is made through a swap [Deposit Channel](/brokers/broker-deposit-channels-swaps#deposit-channels) linked to their BTC address.
...
3. **After 3 Ethereum blocks**, the deposit transaction is considered **final in the State Chain**, and is expected to be executed in about 48 seconds (8 Chainflip State Chain blocks).
...
1. A user initiates a swap by depositing **1 BTC** **to buy** **ETH**. The deposit is made through a swap [Deposit Channel](/brokers/broker-deposit-channels-swaps) linked to their ETH address.
...
Chainflip allows 2 forms of slippage protection, **minimum price**, and **maximum oracle slippage**. This makes sure the user gets a satisfactory result for the swap or you get a refund.

For this reason, the protocol requires a **refund address** and a **retry duration** to be provided when initiating a swap.
A retry duration is the time window (measured in blocks) during which a swap must be executed before the deposit is refunded to the refund address.
If one of the provided forms of protection are not satisfied for the specified retry duration, the deposited assets will be refunded to the specified refund address on the source chain.

These protections are enforced on the AMM level and, unlike network and broker fees, do not take deposit fees and broadcast fees into account as these are charged outside of the AMM layer. That is, if the network fee is 10bps and the broker fee is 15bps, the user will typically see oracle slippage of at least 25bps (but likely larger as we need to account for LPs' spread).
If the deposit was boosted, the boost fee is not refunded.

### Minimum accepted price

If a minimum accepted price is provided, the Chainflip protocol will only swap the deposited assets if the available liquidity leads to a price that is equal or better than the specified minimum price.

### Maximum oracle price slippage

Aka Live price protection. In units of **basis points**.
If provided, the Chainflip protocol will only swap the deposited assets if the output will be within the specified deviation from the oracle price.
This form of protection is not available to all assets, opening a deposit channel or encoding vault params with a maximum oracle slippage for an asset that is not supported will return an error.
...
If [slippage protection](#slippage-protection) is enabled, the minimum price requirement is applied to each individual chunk.
...
4. At block **104** the second chunk is executed at a price of **49,900 USDC**, which is below the limit, and the swap is reverted.
   According to [slippage protection](#slippage-protection) parameters the chunk is to be retried a few times before the swap is refunded,
...
4. As in the example above, the second chunk fails at block **104** and is retried at block **109**.
   ...
   As a result, the amount of **50100 USDC** from the successfully executed chunks is sent to the user's _destination_ USDC wallet address.
   The remaining **4 BTC** is refunded to the user's _refund_ BTC wallet address.
...
<Callout type="info">
  For example, assuming a minimum chunk size of 1,000USDC: If the broker opens a channel requesting
  100 chunks, but the user deposits 10,000USDC, the protocol will reduce the number of chunks to 10.
</Callout>
...
Learn more about the need of a common pair and [the role of USDC in Chainflip's AMM](/protocol/usdusdc-denominated-pools).
