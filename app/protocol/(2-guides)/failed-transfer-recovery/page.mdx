...
To reduce transaction fees for users, the Chainflip protocol will always try to send swap outputs in batches. The protocol also sets a limit on the fees it is willing to pay for each transfer. This is to prevent attacks on the protocol that could drain or grief the protocol.
...
There are multiple reasons why a Transfer can fail, even if they are all very unlikely:
- In EVM chains, a native asset transfer (e.g. Eth) can fail if the receiving address is a smart contract that executes logic consuming more than 8000 gas on receive. Regular multisigs are supported.
- In EVM chains, an ERC-20 token asset transfer (e.g. USDC) can fail if the receiver is blacklisted by the asset issuer.
- In Tron, a TRX asset transfer to a smart contract is not supported. The destination address must be an EOA (no code).
