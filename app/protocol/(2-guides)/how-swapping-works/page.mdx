...
Either through the [Swapping App](https://swap-perseverance.chainflip.io/) or the [Chainflip SDK](/brokers/javascript-sdk), there are two methods to start a Chainflip swap:
...
- Whenever the swap input is first denominated in USD, the Chainflip Network fee is deducted. This could be at the start, middle, or end of swap processing depending on the route. This always occurs at some stage in the swapping process as a function of having [USDC denominated pools](/protocol/usdusdc-denominated-pools "mention").
...
| Network Fee for Buy/Burn Mechanism                              | 0.10% - 0.125%                                                     | Fixed Network Fee used by the protocol to buy FLIP tokens from the USDC-FLIP pool on the State Chain, which in turn are burnt. This includes 0.1% base network fee plus 0.025% from the Boost fee contributions. |
