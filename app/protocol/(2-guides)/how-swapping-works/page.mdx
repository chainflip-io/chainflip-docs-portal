...
Once a method is chosen, the user can send their funds on the external chain (ie. BTC, Ethereum) to be swapped in accordance with their chosen method.
...
| Network Fee for Buy/Burn Mechanism                              | 0.10% (0.01% for stablecoins) $0.5 USD minimum                     | Fixed Network Fee used by the protocol to buy FLIP tokens from the USDC-FLIP pool on the State Chain, which in turn are burnt.                                                                                   |
...
| Refund Fee (If Applicable)                                      | Same as network fee minimum for the asset ($0.5 USD).              | If part or all of a swap request fails, the remaining funds are returned to the user minus the refund fee. Still subject to Broadcast fees.                                                                      |
