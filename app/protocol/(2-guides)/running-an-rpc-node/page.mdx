---
title: Running Full or Archive node
...
## Running a Full or an Archive RPC node
...
For faster startup, first download the latest chaindata snapshot as explained in the [Using Chaindata Snapshots](#using-chaindata-snapshots) section above.
...
NOTE: The configuration above uses `--sync=full` with snapshot data for optimal performance. For archive nodes, see [Setting up an Archive Node](#setting-up-an-archive-node).
