...
Besides the general [implementation checklist](/protocol/cross-chain-messaging/overview#implementation-checklist), there are some specific considerations for EVM chains.
...
- If the gas budget requested is not enough to cover the gas costs of the receiver's logic on the destination chain, the transaction won't be broadcasted. An example of a gas estimation is provided [below](#gas-budget-estimation).
...
- CCM swaps to Tron (TVM) work identically to EVM, except energy is used instead of gas.
- The gas limit on EVM destination chains is capped to 10 million gas. In Tron the fee limit is capped to 1k TRX.
...
| ----------- | ----------- | --------- |
| `srcChain`  | Source [chain ID](/brokers/vault-swaps-api/encoding-reference#supported-chain-and-asset-notation) for the swap. | `uint32`   |
...
The user-provided gas budget value is the amount of gas alllocated for the user logic in the CCM egress transaction on the destination chain. The simplest way to estimate the gas needed on the destination chain is to simulate the execution of the receiver's logic with a direct call to the receiver contract with the parameters expected from the Chainflip call.

#### Ethereum and Arbitrum

Gas is estimated using the `eth_estimateGas` call. The base EVM gas can be subtracted as it will automatically be added by the protocol as part of the overhead. 
...
The analogous estimation can be done on Tron using the `wallet/triggerconstantcontract` HTTP API endpoint, which simulates the contract call and returns the energy consumed.
...
2. The user requests a bitcoin deposit address as described [here](/brokers/javascript-sdk/functions#requestdepositaddressv2) (only when the source chain is EVM-compatible, this can be initiated via a smart contract call).
...
![Signed Transaction Payload](/ccm/signed_transaction_payload.png)
