...
where `MAX_LENGTH` is:
  - 814 bytes for CCM swaps to native SOL. 
  - 725 bytes for CCM swaps to Solana USDC.

If that condition is not met the opening of a deposit channel and the encoding of a Vault Swap will fail.

<Callout type="info">
The upcoming 1.10 release will substantially increase the amount of data that can be passed in a cross-chain message to Solana.
...
| `remaining_accounts` | Destination [asset ID](../vault-swaps/vault-swaps.mdx#supported-chain-and-asset-notation). | Pubkey[] |
