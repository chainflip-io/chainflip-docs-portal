...
This economic security is discussed in more detail on the [Governance & Security](/protocol/governance-and-security 'mention') page.
...
Due to the cap of 150 Authority Validators imposed upon the protocol due to [FROST signature scheme](/protocol/frost-signature-scheme) scalability limitations, the [Validator, Auctions, Bond and Rewards](/validators/validator-auctions-bonds-and-rewards) will determine the amount required to bid through market dynamics every few days.
...
Backup Validators are normally the top 50 bidding non-Authorities (see [Auction Theory](/validators/validators-role#the-auction-system-allocating-validator-slots)) and receive rewards from a limited budget of 1% per year target rate. The budget for rewards is distributed on a pro-rated basis, meaning those backup validators with a higher relative stake to the other Backup Validators will receive more rewards.
...
1. A base network fee of 0.1% taken in USDC on _every swap_, regardless of the number of pools (legs) involved
2. A 0.025% contribution from the Boost pools (introduced in v1.8), taken on every _boosted_ Bitcoin deposit
...
For more information on these fees check how they apply to [Brokers](/brokers#brokers) and [Liquidity Providers](/lp#liquidity-providers).
...
To read more about these penalties, you can check out [Reputation and Slashing](/validators/reputation-and-slashing 'mention')
