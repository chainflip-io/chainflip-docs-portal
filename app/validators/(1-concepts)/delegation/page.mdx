...
description: Delegation guide for operators
---
...
Validators can accept [delegated FLIP](/protocol/delegation) to increase their chances of winning Authority slots during the [Auction](/validators/validator-auctions-bonds-and-rewards).
...
Most of the following functionality is accessible via the [Operator App](https://auctions.chainflip.io/operators).

Operators manage some settings related to delegation:

- `fee_bps`: `integer`
  This is the fee that will be taken from the delegators' share of rewards, measured in basis points (0.01%).
...
4. Each Validator now must accept the Operator's claim by submitting the `accept_operator` extrinsic. This can be done using the `chainflip-cli` on the Validator node:
