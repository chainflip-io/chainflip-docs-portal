...
Bidding is done from a Validator's State Chain account, which is funded through the [State Chain FLIP Gateway](/protocol/funding-state-chain-account#state-chain-flip-gateway).
...
In order for a node to retire, it must enter a non-bidding state before the next Auction begins. See [Validator Types & States](/validators/validator-types-and-states "mention") section for more information.
...
2. Sort all Qualified nodes by descending bid. These are the Auction Bidders. Non-Qualified Nodes are never considered in an Auction (see [Validator Types & States](/validators/validator-types-and-states))
