...
title: Common Problems on Mainnet

description: Common Problems on Mainnet so that you can troubleshoot your node.
...
Please make sure you followed all the steps outlined in [Funding](/validators/mainnet/funding#funding--bidding).
...
Caused by:
    Inconsistent chain configuration. Terminating.Expected ETH chain id 1, received 11155111 through WebSocket., Expected ETH chain id 1, received 11155111 through HTTP.
```

### Solution

You have not selected the correct Ethereum network. In this case, you have selected `sepolia`. The Chainflip network uses Ethereum Mainnet. Please reread [#pointing-to-the-ethereum-client](/validators/mainnet/validator-setup#pointing-to-the-ethereum-client).
...
You have copied the `chainflip-engine` config without changing any of the values! Please re-read the steps in [Engine Settings ](/validators/mainnet/validator-setup#5-engine-settings). You need to make that you use the URL from your Ethereum Client. [#pointing-to-the-ethereum-client](/validators/testnet/validator-setup#pointing-to-the-ethereum-client).
...
Likely you haven't completed all the steps. Please start from [Generating Keys](/validators/mainnet/validator-setup#validator-keys) section and work your way very slowly, making sure you have read everything.

It can be, that you did not successfully submit the `chainflip-cli` commands. Please run all the commands in [Funding and Bidding](/validators/mainnet/funding#funding--bidding) section.
...
You did not successfully submit the `chainflip-cli` commands. Please run all the commands in [Funding and Bidding](/validators/mainnet/funding#register-node-as-a-validator) section.
...
Not all Tron RPC providers support historical balance queries on Mainnet. Use a provider from the supported list below:

| Provider | Mainnet |
|---|---|
| [Tatum](https://tatum.io/) | ✅ Supported |
| [Chainstack](https://chainstack.com/) | ✅ Supported |
| [Quicknode](https://www.quicknode.com/) | ✅ Supported |
| [Trongrid](https://www.trongrid.io/) | 🚫 No historical balance support |
...
```
https://x-api-key:YOUR_API_KEY@your-tron-mainnet-endpoint.com
```
...
Before integrating, you can verify whether your provider supports `getBlockBalance` with the following curl command. Replace `<your-tron-rpc-endpoint>` with your provider's Mainnet endpoint:

```bash copy
curl -X POST https://<your-tron-rpc-endpoint>/wallet/getblockbalance \
  -H "Content-Type: application/json" \
  -d '{"number": 77096758, "hash": "0000000004986736812cbf15ffbcdd229bd3d76a595db895719867cc2da3a5bd", "visible": true}'
```
...
Follow instructions under [Submitting an issue](/validators/mainnet/submitting-an-issue).
