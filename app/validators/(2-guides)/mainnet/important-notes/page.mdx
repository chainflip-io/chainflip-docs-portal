---
title: Important Notes on Mainnet

description: Important notes about the Chainflip network on Mainnet; how to properly secure the Chainflip Protocol and Validators.
---
...
- [Concepts](/protocol)
- [The Validator Network](/protocol/protocol-overview#the-state-chain)
...
Backing up and securing the Validator's `$FLIP` bond is akin to backing up any cryptocurrency wallet. The seed-phrase that was created by [generating validator keys](/validators/mainnet/validator-setup#validator-keys) must be securely and remotely stored; ideally in a physically secure location offline. This seed-phrase will be required to recover your Chainflip account keys and your Ethereum keys. This is the absolute minimum requirement to backing up your Validator's `$FLIP` bond.
...
If you _do_ lose your session keys irrecoverably, run `sudo chainflip-cli --config-root /etc/chainflip rotate` to generate and register a fresh key (assuming you still have access to the VPS and the data is not corrupted). However note that key cannot author blocks until the following epoch.
...
If you _do_ lose access to these keys, you can [recover your validator account](/validators/mainnet/validator-setup#recovering-your-keys) and restore it to a state where it can participate in the network again.

However, there are important considerations to keep in mind:

- These keys are separate from the session keys and engine key shares. The latter are _not_ recoverable from a seed.
...
- If you think the keys might be compromised, we recommend that you immediately secure your `$FLIP` funds by [binding a redeem address](/validators/mainnet/funding#redemption-execution-address-binding), [stopping your bid](/validators/mainnet/funding#stop-bidding) and then, when your funds are unbonded, redeeming them to your wallet before funding a new node with new keys.
...
Note it is _not_ possible to recover these key shares from a seed if they are lost, so it it crucial to back up this database, ideally at the beginning of each epoch.
