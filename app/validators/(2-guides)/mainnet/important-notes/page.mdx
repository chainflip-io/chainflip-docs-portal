...
Backing up and securing the Validator's `$FLIP` bond is akin to backing up any cryptocurrency wallet. The seed-phrase that was created by [generating validator keys](keys#validator-keys) must be securely and remotely stored; ideally in a physically secure location offline. This seed-phrase will be required to recover your Chainflip account keys and your Ethereum keys. This is the absolute minimum requirement to backing up your Validator's `$FLIP` bond.
