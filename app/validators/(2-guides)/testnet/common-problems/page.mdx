...
title: Common Problems on Testnet

description: Common Problems on Testnet so that you can troubleshoot your node.

---

import { Callout } from "nextra/components";

# Common Problems on Testnet
...
Please make sure you followed all the steps outlined in [Funding](/validators/testnet/funding).
...
You have not selected the correct Ethereum network. In this case, you have selected `mainnet`. The Chainflip testnet network uses Sepolia. Please reread [#pointing-to-the-ethereum-client](/validators/testnet/validator-setup#pointing-to-the-ethereum-client).
...
You have copied the `chainflip-engine` config without changing any of the values! Please re-read the steps in [Engine Settings ](/validators/testnet/validator-setup#5-engine-settings). You need to make that you use the URL from your Ethereum Client. [#pointing-to-the-ethereum-client](/validators/testnet/validator-setup#pointing-to-the-ethereum-client).
...
You either need to resize your server to have more RAM or create a swapfile (8GB should be enough), you can follow this guide in order to do so: https://linuxize.com/post/create-a-linux-swap-file/
Having a swapfile or swap partition is recommended in any case.
...
<br />
<img src="/testnet/validator_offline.png" alt="validator offline" />
<br />
...
<br />
<img src="/testnet/validator_authority.png" alt="validator online" />
<br />

You need to wait until the next Rotation. Make sure your node is <span style={{ color: "green"}}> <strong> Online </strong> </span> with the labels **Bidding** and **Qualified**.

### Authority Offline — Reasons

<br />
<img src="/testnet/validator_authority_offline.png" alt="" />
<br />
...
### Bidding but not Qualified — Reasons

You did not successfully submit the `chainflip-cli` commands. Please run all the commands in [Funding and Bidding](/validators/testnet/funding#registering-and-setting-up-your-validator-node) section.

The commands that already ran successfully will fail with an error resembling the following:
<br />
<img src="/testnet/validaotr_cli_commands_failing.png" alt="validator cli commands failing(command already executed before)" />
<br />

This is expected, continue with the next command until you complete them all, after completing this your node should appear **Bidding** and **Qualified**.
...
<img src="/testnet/image (9).png" alt="" />
...
## Engine Fails to Start — Tron RPC Missing Historical Balance Support
...
The Chainflip engine requires the Tron RPC node to support historical balance queries (`getBlockBalance`). If the Tron endpoint does not have this feature enabled, the engine will fail to start.

### Solution

#### Using an RPC Provider

Not all Tron RPC providers support historical balance queries on Nile testnet. Use a provider from the supported list below:

| Provider | Nile Testnet |
|---|---|
| [Tatum](https://tatum.io/) | ✅ Supported |
| [Trongrid](https://www.trongrid.io/) | ✅ Supported |
| [Chainstack](https://chainstack.com/) | 🚫 No historical balance support |
| [Quicknode](https://www.quicknode.com/) | 🚫 No Nile testnet support |

#### RPC Provider Authentication

The Chainflip engine does not support passing authentication headers to RPC endpoints. If your provider requires authentification via an `x-api-key` header, it is usually supported to embed the API key directly in the URL using Basic Authentication format:

```
https://x-api-key:YOUR_API_KEY@your-tron-nile-endpoint.com
```

Use this URL format when configuring the Tron RPC endpoint in your engine settings (`/etc/chainflip/config/Settings.toml`).

#### Testing Your Tron RPC Endpoint

Before integrating, you can verify whether your provider supports `getBlockBalance` with the following curl command:

```bash copy
curl -X POST https://<your-tron-rpc-endpoint>/wallet/getblockbalance \
  -H "Content-Type: application/json" \
  -d '{"number": 64843264, "hash": "0000000003dd6e006934d46981dab0f3cf1863b6d7b0a50577e198e06bb8560b", "visible": false}'
```
...
{
  "block_identifier": {
    "hash": "0000000003dd6e006934d46981dab0f3cf1863b6d7b0a50577e198e06bb8560b",
    "number": 64843264
  },
  "timestamp": 1770990702000,
  "transaction_balance_trace": [...]
}
```

If historical balance queries are not supported, the response will be:

```json
{"Error":"class org.tron.core.exception.ItemNotFoundException : This block does not exist"}
```

#### Running Your Own Tron Node

If you are running a self-hosted `java-tron` full node, historical balance queries must be explicitly enabled in your node's `config.conf` file. Add or update the following settings:

```conf
storage {
  historyBalanceLookup = true
}
```

After updating the configuration, restart the Chainflip engine:
...
Follow instructions under [Submitting an issue](/validators/testnet/submitting-an-issue).
...
Thus, we have a `FailedToWitnessInTime` penalty. This penalty is dispatched after the following:

1. A call that requires witness is triggered on the external chain.

2. Some nodes have witnessed the call and make the `witness_at_epoch` extrinsic call.

3. After a certain amount of time has passed, a certain threshold has been reached (currently 2/3), the call is dispatched. At this moment, a deadline is set for all remaining validators - the call must be witnessed by the remaining nodes before this deadline is up. This deadline (`current_block` + `LATE_WITNESS_GRACE_PERIOD`) is currently set at **10** blocks.

4. When the deadline is reached, all nodes that have not witnessed this specific call will be reported with the offense of `FailedToWitnessInTime`, and their reputation will be deducted accordingly.
...
1. From your authority's page on scan.chainflip.io and under the "Penalties" tab you will see the block number associated with `FailedToWitnessInTime`. Click on that to see all the events in the block. 

2. Search the page for `ReportedWitnessingFailures`. There may be multiple. Expand these and find if your account id is listed under any of them. Note this block number and call hash.

3. Go to the block number specified using the search bar of the explorer.

4. Search the page for that hash. The fastest would be to inspect the page and search the HTML for the hash - Note that if you're not witnessing chain-tracking for a particular chain, this process won't help. So if you don't have a match, it's probably chain tracking which is your issue. Which could mean one of your RPC node is slow.
