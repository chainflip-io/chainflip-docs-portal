...
<Callout type="warning">
    Please ensure that all connections on ports `30333` and `8078` are not
    blocked! Some VPS providers enable an all-port open firewall by default, but
    if this is not the case you need to open such ports, otherwise your node
    will not be able to participate in Keygen / Signing ceremonies. **As a
    result of this you node will never become active and you'll be slashed and
    might lose all your `tFLIP`**.
</Callout>
...
If this succeeds, open the Auctions app, look under `Current Auction State`, disable `Hide unqualified nodes`, and search for your node ID in the search box. Your node should appear with the `Bidding` label. After the next rotation, a healthy node should move toward `Qualified`, and later `Authority` if selected.
...
- `Qualified`: the validator is healthy and eligible, but not currently selected as an authority
